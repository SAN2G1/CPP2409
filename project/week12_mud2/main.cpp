...
int main()
{
	...
	{ // 사용자에게 계속 입력받기 위해 무한 루프
		...
		// 사용자의 입력을 저장할 변수
		string user_input = "";
		cout << "명령어를 입력하세요 (w(up),s(down),a(left),d(right),map,info,exit): ";
		cin >> user_input;
		// info 여부와 종료 여부를 체크한다.
		if (user_input == "info") { cout << player1; continue;}
		else if (user_input == "exit") { cout << "종료합니다." << endl; break; }
		
        if( !CheckUser(player1))  {cout << "체력이 0이 되어 종료합니다. 실패했습니다." << endl; break;}
		// moving함수를 호출하고 그 반환 값이 true이면 다음행동을 진행하고 false면 다시 입력을 받는다.
		if (!moving(user_input, player1, map, user_x, user_y))
		{
			cout << "잘못된 입력입니다. 다시 돌아갑니다." << endl;
			continue;
		}
		displayMap(map, user_x, user_y);
		checkState(map, player1, user_x, user_y); 
		// 목적지에 도달했는지 체크
...
bool moving(string user_input, User &user, vector<vector<int>> map, int &user_x, int &user_y)
{
	...
	else
	{
		...
		user_y = buf_y;
		if (index == 0)
			user.IncreaseHP(1); // map이나 exit과 같이 제자리 움직임의 경우 hp를 다시 회복시켜줌.
		return true;
	}
